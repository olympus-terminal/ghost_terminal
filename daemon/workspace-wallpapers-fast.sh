...
set_wallpaper_fast() {
    local wallpaper="$1"
    # Use dconf to directly write to GNOME's settings database for immediate effect
    dconf write /org/gnome/desktop/background/picture-uri "'$wallpaper'"
    dconf write /org/gnome/desktop/background/picture-uri-dark "'$wallpaper'"
...
monitor_workspace_changes() {
    ...
    while true; do
        ...
        if [ "$CURRENT_WORKSPACE" != "$LAST_WORKSPACE" ]; then
            ...
            if [ -n "${WALLPAPERS[$CURRENT_WORKSPACE]}" ]; then
                ACTUAL_WALLPAPER=$(dconf read /org/gnome/desktop/background/picture-uri | tr -d "'")
...
if [ -z "$IMAGE_DIR" ]; then
    IMAGE_DIR="$DEFAULT_BASE_DIR/MJ7-Topaz/light-processed"
fi
